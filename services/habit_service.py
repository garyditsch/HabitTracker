...
def get_all_habits(include_private=False):
    ...
    if include_private:
        query = """
            SELECT id, name, is_active, is_public, order_index, created_at
            FROM habits
...
def create_habit(name, is_public=True):
    """
    ...
    query = """
        INSERT INTO habits (name, is_public, is_active, order_index)
        VALUES (?, ?, 1, ?)
    """
    return db.execute_update(query, (name, 1 if is_public else 0, next_order))
